...
namespace MiniORM
{
	public abstract class DbContext
    {
        ...
        internal static readonly Type[] AllowedSqlTypes = { typeof(string), typeof(int), typeof(uint), typeof(long), typeof(ulong), typeof(decimal), typeof(bool), typeof(DateTime) };
        ...
        public void SaveChange()
        {
            ...
            foreach (IEnumerable<object> dbSet in dbSets)
            {
                ...
                {
                    throw new InvalidOperationException("{invalidEntities.Length} Invalid Entities found in {dbSet.Name}!");
                }
            }
        ...
        private void MapCollection<TDbSet, TCollection>(DbSet<TDbSet> dbSet, PropertyInfo collectionProperty) where TDbSet : class, new () where TCollection : class, new ()
        {
            ...
            foreach (var entity in dbSet)
            {
                ...
                var navigationEntities = navigationDbSet.Where(n => primaryKey.GetValue(n).Equals(primaryKeyValue)).ToArray();
        ...
        private void MapNavigationProperties<TEntity>(DbSet<TEntity> dbSet) where TEntity : class, new ()
        {
            ...
            foreach (var foreignKey in foreignKeys)
            {
                ...
                foreach (var entity in dbSet)
                {
                    ...
                    var navigationPropertyValue = ((IEnumerable<object>)navigationDbSet).First(cp => navigationPrimaryKey.GetValue(cp).Equals(foreignKeyValue));
